...
function getUsersArray($es_) {
    ...
    if (0<$es_["user_id"] && $es_["user_id"]<1) {
        $user = [];
        $id = $es_["user_id"];
        $user[$id]["user"] = $es_["user"];
        $user[$id]["name"] = $es_["user_name"];
        $user[$id]["password"] = "login_from_facebook";
//        echo "<pre>";
//        var_dump($user);
//        echo "</pre>";
...
function getLoginForm ($sessionInfo) {
    ...
    if ($sessionInfo['user_id']>=0 ) {
        ...
        $loginTemplate = getSourceContent($fileName);
        $loginTemplate = str_replace('{{ID}}', ID, $loginTemplate);
        $loginTemplate = str_replace('{{URL}}', URL, $loginTemplate);
    }
...
function getNavigation($session_data) {
    $navigationFileName = NAVIGATION_SOURCE_PATH;
    $navigationData = getSourceData($navigationFileName);
    if (empty($navigationData)) {
        return '';
    }

    $navigationTemplateName = NAVIGATION_TEMPLATE_PATH;
    ...
    $linksFileName = NAVIGATION_TEMPLATE_LINKS_PATH;
